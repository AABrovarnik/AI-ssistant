...
## 2026-08-18 — Просроченные задачи можно завершать из любого активного статуса

Context:

- Просроченность определяется по `due_at` и не переводит запись автоматически
  в отдельный статус `OVERDUE`.
- Поэтому личная задача могла отображаться как просроченная, оставаясь в
  `NEW` или `POSTPONED`, а кнопка завершения возвращала ошибку перехода.

Decision:

- Разрешить переход в `DONE` из всех активных статусов задачи.
- Не менять автоматически существующие задачи и не добавлять миграцию данных.
- Проверять исправление через API/worker restart smoke; закрытие конкретной
  пользовательской задачи выполнять только явным действием владельца.
